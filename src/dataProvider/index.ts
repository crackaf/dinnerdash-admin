...
export default (apiUrl: string, httpClient = fetchUtils.fetchJson) => {
  ...
  const convertDataRequestToHTTP = (
    type: any,
    resource: any,
    params: { pagination: { page: any; perPage: any }; id: any; ids: any[]; data: any },
  ) => {
    let url = '';
   ...
   */
  return (type: string, resource: any, params: { ids: any[]; data: any }) => {
    ...
    const { url, options } = convertDataRequestToHTTP(type, resource, params as any);
